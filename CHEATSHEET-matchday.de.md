...
| `[team] league_shortcuts` | Nur diese Ligen zählen — **exakte** Ligakürzel, z.B. `bl2, dfb, rlw-frauen`. OpenLigaDB liefert vereinzelt Fantasie-Ligen mit falschen Terminen (real gesehen: **„ESP8266"**) und führt dieselbe Partie in Varianten derselben Liga (`bl2h` neben `bl2`) an verschiedenen Tagen. Verworfene Ligen stehen im Log. Der Vorgänger `league_prefixes` verglich Präfixe und gilt weiter, solange dieser Schlüssel leer ist. |
