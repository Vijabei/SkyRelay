...
| `[team] league_shortcuts` | Nur diese Ligen zählen — **exakte** Ligakürzel, z.B. `bl2, dfb, rlw-frauen`. OpenLigaDB liefert vereinzelt Fantasie-Ligen mit falschen Terminen (real gesehen: **„ESP8266"**) und führt dieselbe Partie in Varianten derselben Liga (`bl2h` neben `bl2`) an verschiedenen Tagen. Verworfene Ligen stehen im Log. Der Vorgänger `league_prefixes` verglich Präfixe; er steht nicht mehr in der Vorlage, wird aber weiter gelesen und gilt, solange dieser Schlüssel leer ist. |
...
| `⚠️ Kein DFL-Kürzel für "XY"` im Log | Pokal-/unbekannter Gegner → korrektes Kürzel unter `[team_codes]` nachtragen. |
...
| Ticker startet an einem spielfreien Tag | Sollte durch `league_shortcuts` verhindert sein. Log prüfen auf „matches from other leagues ignored" bzw. welche Liga als Spieltag erkannt wurde. |
...
Raspberry Pi mit **64-Bit-System** (aarch64). Installiert und aktualisiert wird
über die Skripte, nicht von Hand:
...
Die Paketstände stehen in `requirements.txt` — `neonize` ist dort bewusst
